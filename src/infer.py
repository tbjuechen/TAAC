...
from model import (
    PCVRHyFormer,
    ModelInput,
    PAIR_WEIGHTED_FIDS_COUNT,
    PAIR_WEIGHTED_FIDS_SCORE,
)
...
#
# Special note on ``pair_weight_mode``: trainer dumps train_config.json from
# ``vars(args)`` (see train.py: train_config=vars(args)), so the on-disk key is
# the CLI flag name ``pair_weighted_pool``, NOT the model constructor name
# ``pair_weight_mode``. ``resolve_model_cfg`` translates between them.
_FALLBACK_MODEL_CFG = {
    'd_model': 64,
    'emb_dim': 64,
    'num_queries': 1,
    ...
    'user_ns_tokens': 0,
    'item_ns_tokens': 0,
    # W2.6 (v1 weighted pool / v2 PairSetEncoder). 'none' = baseline mean-pool.
    # When training used pair_weighted_pool='transformer', ckpt has additional
    # pair_set_encoders.* sub-modules that strict load_state_dict requires.
...
# Optional: longer encoder gather side (default head). Read from train_config
# when present so longer-encoder ckpts load with the same wiring used at train.
_FALLBACK_LONGER_GATHER_SIDE = 'head'
...
def resolve_model_cfg(train_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    1. ``num_time_buckets``: not exposed on the CLI as an independent
       hyperparameter; the bucket count is uniquely determined by the length
       of ``dataset.BUCKET_BOUNDARIES``. Resolution order:
         a) ``train_config`` contains ``num_time_buckets`` directly (legacy ckpt)
            -> use that value;
         b) ``train_config`` contains ``use_time_buckets`` (new-style training)
            -> derive as ``NUM_TIME_BUCKETS`` or ``0``;
         c) neither is present -> fall back to ``_FALLBACK_MODEL_CFG[...]``.

    2. ``pair_weight_mode``: train_config dumps ``vars(args)`` so the on-disk
       key is the CLI flag name ``pair_weighted_pool``. Resolution order:
         a) ``pair_weight_mode`` present (rare; only if explicitly written)
            -> use that;
         b) ``pair_weighted_pool`` present (normal case from train.py)
            -> use that;
         c) neither -> fall back to ``_FALLBACK_MODEL_CFG[...]`` ('none').

       When mode != 'none', ``build_model`` will additionally pass
       ``user_paired_dense_specs`` / ``user_int_fids`` to PCVRHyFormer so the
       state_dict's ``pair_set_encoders.*`` keys map correctly.
    """
    cfg: Dict[str, Any] = {}
    for key in _MODEL_CFG_KEYS:
        ...
        if key == 'pair_weight_mode':
            # train_config dumps vars(args) → CLI key is 'pair_weighted_pool'
            if 'pair_weight_mode' in train_config:
                cfg[key] = train_config['pair_weight_mode']
            elif 'pair_weighted_pool' in train_config:
                cfg[key] = train_config['pair_weighted_pool']
            else:
                cfg[key] = _FALLBACK_MODEL_CFG[key]
                logging.warning(
                    f"train_config missing both 'pair_weight_mode' and "
                    f"'pair_weighted_pool', using fallback = {cfg[key]}")
            continue
...
def build_model(
    ...
    Args:
        ...
        model_cfg: resolved model hyperparameters, typically the output of
            ``resolve_model_cfg``.
        train_config: optional raw train_config.json dict. Used to look up
            non-``_MODEL_CFG_KEYS`` extras (e.g. longer_gather_side).
    ...
    # ---- W2.6 pair-weighted pool wiring (mirrors train.py:329-348) ----
    # When pair_weight_mode != 'none', the trained model has additional
    # pair_set_encoders.{fid_idx}.* sub-modules in its state_dict. We must
    # rebuild the same dispatch state on inference so strict load_state_dict
    # succeeds and the same forward path is used.
    #
    # Specs are derived from the dataset schema (NOT serialized in
    # train_config.json). As long as the schema matches training (it does:
    # infer.py copies schema.json from model_dir or falls back to data_dir),
    # the fid ordering and slice offsets match → fid_idx → state_dict keys
    # line up.
    user_paired_dense_specs: Dict[int, Tuple[int, int]] = {}
    user_int_fids: List[int] = [
        fid for fid, _doff, _dlen in dataset.user_int_schema.entries
    ]
    pair_weight_mode = model_cfg.get('pair_weight_mode', 'none')
    if pair_weight_mode != 'none':
        wanted = set(PAIR_WEIGHTED_FIDS_COUNT) | set(PAIR_WEIGHTED_FIDS_SCORE)
        for fid, doff, dlen in dataset.user_dense_schema.entries:
            if fid in wanted:
                user_paired_dense_specs[fid] = (doff, dlen)
        logging.info(
            f"Pair-weighted pool inference mode={pair_weight_mode}; "
            f"paired fids: {sorted(user_paired_dense_specs.keys())}"
        )

    # ---- Optional extras from train_config (not in _MODEL_CFG_KEYS) ----
    # seq_longer_gather_side: only meaningful when seq_encoder_type='longer'.
    # Naming asymmetry: PCVRHyFormer kwarg is `seq_longer_gather_side` (with
    # prefix); train_config.json stores the CLI flag name `longer_gather_side`
    # (no prefix; train.py translates internally before calling the model).
    # We do the same translation here.
    extra_kwargs: Dict[str, Any] = {}
    if train_config is not None:
        gather_side = (
            train_config.get('seq_longer_gather_side')        # rare: explicit model key
            or train_config.get('longer_gather_side')         # normal: CLI key from vars(args)
            or _FALLBACK_LONGER_GATHER_SIDE
        )
        extra_kwargs['seq_longer_gather_side'] = gather_side

    logging.info(f"Building PCVRHyFormer with cfg: {model_cfg}, extras: {extra_kwargs}")
    model = PCVRHyFormer(
        ...
        # W2.6 pair-weighted pool wiring (mode goes through model_cfg below).
        user_paired_dense_specs=user_paired_dense_specs,
        user_int_fids=user_int_fids,
        **model_cfg,        # contains pair_weight_mode + all other model hparams
        **extra_kwargs,     # contains longer_gather_side
    ).to(device)
...
def load_model_state_strict(
    ...
    try:
        ...
        logging.error(
            ...
            "the training hyperparameters (pair_weighted_pool, seq_encoder_type, "
            "longer_gather_side, etc.).")
        raise e
