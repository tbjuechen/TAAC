...
# RankMixer token-specific FFN:    RANK_MIXER_PER_TOKEN_FFN=1 ./run.sh
...
RANK_MIXER_PER_TOKEN_FFN="${RANK_MIXER_PER_TOKEN_FFN:-0}"
if [ -z "${D_MODEL+x}" ]; then
...
[ "${RANK_MIXER_PER_TOKEN_FFN}" = "1" ] && EXTRA_FLAGS="${EXTRA_FLAGS} --rank_mixer_per_token_ffn"
