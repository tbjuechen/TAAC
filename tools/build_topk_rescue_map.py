...
Input can be either:
  1) a topk_tail_eda.json that contains a full "topk_map" object, produced
     with tools/topk_tail_eda.py --export-map-targets ... --export-topk-map ...
  2) a standalone topk rescue map JSON with a top-level "targets" object.

The plain EDA report without --export-topk-map only stores top_ids_preview and
is intentionally rejected because it does not contain enough ids for top10k or
top200k remapping.
"""
...
def extract_source_map(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if isinstance(payload.get("targets"), list):
        raise ValueError(
            "input looks like a plain EDA report without full topK ids. "
            "Rerun tools/topk_tail_eda.py with --export-map-targets and "
            "--export-topk-map, then feed that JSON here.")
...
def main() -> None:
    ...
    parser.add_argument("--print-one-line", action="store_true",
                        help="Print the whole map as one JSON-escaped log line")
    args = parser.parse_args()

    with open(args.eda_json, "r", encoding="utf-8") as f:
        payload = json.load(f)
    source_map = extract_source_map(payload)
    result = build_map(source_map, parse_targets(args.targets))
